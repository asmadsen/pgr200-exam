...
public abstract class HasOneOrMany<V extends BaseRecord<V, W>, W extends IBaseModel<W>, T extends BaseRecord<T, ?>> extends AbstractRelation<V, W, T> {
    ...
    public void addEagerConstraints(Collection<T> models) {
        this.query.where(this.foreignKey, SqlOperator.In, models.stream().map(m -> {
            return m.getState().getAttribute(this.localKey).getValue();
